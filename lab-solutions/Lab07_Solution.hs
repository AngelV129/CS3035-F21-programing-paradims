...
-- YOUR CODE START HERE
squares xs ys = [(x - y)^2 | (x,y) <- zip xs ys]
diffSquared xs ys = (sum (squares xs ys)) / (fromIntegral (length (squares xs ys)))

squares' xs ys = [(fst ps - snd ps)^2 | ps <- zip xs ys]
diffSquared' xs ys = (sum (squares' xs ys)) / (fromIntegral (length (squares' xs ys)))

diffSquared'' xs ys = (sum squares'') / (fromIntegral $ length squares'')
    where squares'' = [(x - y)^2 | (x,y) <- zip xs ys]
-- YOUR CODE END HERE
...
largers :: [Int] -> [Int] -> [Int]
-- YOUR CODE START HERE
largers xs ys = [max x y | (x,y) <- zip xs ys]

largers' xs ys = [max (fst ps) (snd ps) | ps <- zip xs ys]

largers'' xs ys = [if fst ps > snd ps then fst ps else snd ps | ps <- zip xs ys]
-- YOUR CODE END HERE
...
twoAhead :: [Int] -> [Int]
-- YOUR CODE START HERE
twoAhead xs = [y-x | (x,y) <- zip xs (drop 2 xs)]

twoAhead' xs = [snd ps - fst ps | ps <- zip xs (drop 2 xs)]
-- YOUR CODE END HERE
