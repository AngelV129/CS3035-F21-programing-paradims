...
takes one value with type $a$ and a list of $a$ type elements, where type $a$
...
-- YOUR CODE START HERE
positions x xs = [i | (y, i) <- zip xs [1..(length xs)], x == y ]
-- YOUR CODE END HERE
...
e.g. $search 7 [1, 2, 3, 4, 5]$ returns $True$
